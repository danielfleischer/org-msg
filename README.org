...
** CSS stylisation
In order to avoid CSS conflict, *OrgMsg* performs inline replacement when it generates the final HTML message.
To customize the style, you can override the ~org-msg-enforce-css~ variable in two ways:
either you define the CSS as a property list (see the default ~org-msg-default-style~ variable for reference) or you load the css using the dedicated helpers:

#+begin_src emacs-lisp
  (setq org-msg-enforce-css (org-msg-css-file-to-list "my/path/to/style.css"))
#+end_src

It is also possible to dump the current style into a CSS file:

#+begin_src emacs-lisp
  (org-msg-css-to-file "my/path/to/style.css")
#+end_src
...
And it will look like this (with the default style).
